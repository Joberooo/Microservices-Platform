...
if (-not $env:JWT_SECRET)
{
    ...
    if (-not $line)
    {
        throw "JWT_SECRET not found in .env and not set in environment"
    }
...
if (-not $?)
{
    throw "Failed to generate JWT"
}
Write-Host ("Token prefix: {0}..." -f $TOKEN.Substring(0, 20))

$base = "http://localhost:${env:GATEWAY_PORT}"; if (-not $env:GATEWAY_PORT)
{
...
if (-not $docsOk)
{
    Write-Warning "OpenAPI docs not reachable (expected 200). Swagger UI may not show operations."
}
